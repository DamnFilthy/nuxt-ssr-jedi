  ...
  <section v-if="this.films !== null && !loading">
    ...
    <div>
      ...
      <ul v-if="this.planetsInFilm.length > 0" class="list-block">
        <nuxt-link  v-for="planet in this.planetsInFilm" :key="index" class="list-item" no-prefetch active-class="active" :to="/planets/ + planet.url.split('/')[5]">
          {{planet.name}}
        </nuxt-link>
      </ul>
      <div v-else>No data...</div>
    ...
    <div>
      ...
      <ul v-if="this.starships.length > 0" class="list-block">
        <nuxt-link  v-for="starship in this.starships" :key="index" class="list-item" no-prefetch active-class="active" :to="/starships/ + starship.url.split('/')[5]">
          {{starship.name}}
        </nuxt-link>
      </ul>
      <div v-else>No data...</div>
    ...
    <div>
      ...
      <ul v-if="this.species.length > 0" class="list-block">
        <nuxt-link  v-for="specie in this.planetsInFilm" :key="index" class="list-item" no-prefetch active-class="active" :to="/species/ + specie.url.split('/')[5]">
          {{specie.name}}
        </nuxt-link>
      </ul>
      <div v-else>No data...</div>
